...
const WordFeedback = ({ word, secretWord }) => {
    return word ? (
        <View style={[tw`flex flex-row my-2`]}>
            {[0, 1, 2, 3, 4].map((pos, j) => (
                <View key={j} style={[tw`w-14 h-14 mx-2 flex flex-row justify-center items-center uppercase font-bold border rounded
        ...
        `]}>
                    <Text style={[tw`uppercase text-3xl text-gray-800 ${word.substring(pos, pos + 1) === secretWord.substring(pos, pos + 1) && "text-white"} ${word.substring(pos, pos + 1) !== secretWord.substring(pos, pos + 1) && secretWord.toLowerCase().indexOf(word.substring(pos, pos + 1)) === -1 && "text-white"}
